...
import { IGetComment, IGetPost } from '../types/types';
...
export const DashboardComponent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const data = await UserService.getUsersWithLimit();
        if (data) {
          setUsers(data.users);
    ...
    const fetchPosts = async () => {
      try {
        const data = await PostService.getPostsWithLimitFive();
        if (data) {
          setPosts(data.posts);
    ...
    const fetchComments = async () => {
      try {
        const data = await CommentService.getCommentsWithLimit();
        if (data) {
